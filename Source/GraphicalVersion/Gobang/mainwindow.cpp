...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent)
{
      setFixedSize(kBoardMargin * 2 + kBlockSize * 15, kBoardMargin * 2 + kBlockSize * 15);
...
{
    // 人下棋，并且不能抢机器的棋
    if (gobang->gameType == PVP || gobang->playerFlag)
    {
        chessOneByPerson();
        // 如果是人机模式，需要调用AI下棋
        if (gobang->gameType != PVP)
...
void MainWindow::initPVEGame(int whoFirst)
{
     ...
     if (whoFirst==PVEwithPlayerFirst)
         gobang->playerFlag=PlayerFirst;
     else if (whoFirst==PVEwithAIFirst)
         gobang->playerFlag=AIFirst;

     gobang->startGame(gobang->gameType);

     if (!gobang->playerFlag) chessOneByAI();
     update();
}
